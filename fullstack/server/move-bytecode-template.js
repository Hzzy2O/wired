...
async function __wbg_init(input) {
  ...
  if (typeof input === 'string') {
    ...
    const { instance, module } = await WebAssembly.instantiate(arrayBuffer, imports);
    return __wbg_finalize_init(instance, module);
  }
  ...
  if (input instanceof Request) {
    const arrayBuffer = await (await fetch(input)).arrayBuffer();
    const { instance, module } = await WebAssembly.instantiate(arrayBuffer, imports);
    return __wbg_finalize_init(instance, module);
  }

  if (input instanceof ArrayBuffer) {
    const { instance, module } = await WebAssembly.instantiate(input, imports);
    return __wbg_finalize_init(instance, module);
  }

  throw new Error('Unsupported input type for wasm initialization');
}
